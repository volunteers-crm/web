export const API_AUTH_CONFIRM = '/auth/telegram'
export const API_AUTH_ME = '/auth/me'

export const API_URL_PAGE = '/pages/:slug'

export const API_APPEALS_INDEX = '/appeals'
export const API_APPEALS_SHOW = '/appeals/:id'
export const API_APPEALS_START_WORK = '/appeals/:id/work'
export const API_APPEALS_PUBLISH = '/appeals/:id/publish'
export const API_APPEALS_DONE = '/appeals/:id/done'
export const API_APPEALS_CANCEL = '/appeals/:id/cancel'
export const API_APPEALS_MESSAGES = '/appeals/:id/messages'

export const API_CHANNELS_INDEX = '/channels'
export const API_CHANNELS_CHANNEL = '/channels/:id'

export const API_BOTS_INDEX = '/bots'
export const API_BOTS_BOT = '/bots/:id'

export const API_ROLES_INDEX = '/roles'
export const API_ROLES_SHOW = '/roles/:id'

export const API_BECOME_INDEX = '/become'
export const API_BECOME_SEARCH = '/become/:id'
export const API_BECOME_STORE = '/become/:id'
export const API_BECOME_CANCEL = '/become/:id'

export const API_BECOMES_NEW = '/becomes'
export const API_BECOMES_ACCEPTED = '/becomes/accepted'
export const API_BECOMES_DECLINED = '/becomes/declined'
export const API_BECOMES_ACTION = '/becomes/:id'
